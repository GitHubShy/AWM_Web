<template>
  <div class="wrapper">
    ...
    <vxe-table border resizable ref="xTable" height="500" :data="aircraft">
      ...
      <vxe-table-column field="maintenance_cycle"
                        title="Maintenance_cycle"></vxe-table-column>
      <vxe-table-column field="last_modify_time"
                        title="LastSerice"></vxe-table-column>
      <vxe-table-column field="next_modify_time"
                        title="nextService"></vxe-table-column>
      <vxe-table-column field="status" title="Status">

        <template v-slot="{ row }">
          <vxe-button :status="setStatus(row)"
                      :content="setStatusText(row)"
                      size="mediam"
                      style="width: 100px"
                      @click="showDetails(row)"></vxe-button>
        </template>
...
<script>
import { getAircraft } from "@n/Workshop";
export default {
  components: {},
  props: {},
  data() {
